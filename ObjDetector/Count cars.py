...
#function to find center of car/truck if given
...
pos_line = int(cap.get(4)/1.25)  # Count Line Position
print(pos_line)
...
while True:
    ...
    for (i, c) in enumerate(contour):
        (x, y, w, h) = cv2.boundingRect(c)#used to highlight region of interest after drawing a countour
        validate_contour = (w >= width_min) and (h >= height_min)
        if not validate_contour:
            continue
        #gives the rectangle a color and gives the center of the rectangle a dot
        cv2.rectangle(frame1, (x, y), (x + w, y + h), (0, 255, 0), 2)
        center = catch_center(x, y, w, h)
        detect.append(center)
        cv2.circle(frame1, center, 4, (255, 0, 0), -1)
        #if a car reaches the line(the center dot overlaps the line's y position)then the rectangle dissappears and
        for (x, y) in detect:
